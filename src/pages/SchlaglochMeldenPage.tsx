...
export default function SchlaglochMeldenPage() {
  const [records, setRecords] = useState<SchlaglochMelden[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<SchlaglochMelden | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<SchlaglochMelden | null>(null);
  const [viewingRecord, setViewingRecord] = useState<SchlaglochMelden | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getSchlaglochMelden());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: SchlaglochMelden['fields']) {
    await LivingAppsService.createSchlaglochMeldenEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: SchlaglochMelden['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateSchlaglochMeldenEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Schlagloch melden"
      subtitle={`${records.length} Schlagloch melden im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Schlagloch melden suchen..."
          value={search}
          ...
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('melder_email')}>
                <span className="inline-flex items-center gap-1">
                  E-Mail-Adresse (Melder/in)
                  {sortKey === 'melder_email' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('melder_telefon')}>
                <span className="inline-flex items-center gap-1">
                  Telefonnummer (Melder/in)
                  {sortKey === 'melder_telefon' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={10} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Schlagloch melden. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <ConfirmDialog
        ...
        title="Schlagloch melden löschen"
