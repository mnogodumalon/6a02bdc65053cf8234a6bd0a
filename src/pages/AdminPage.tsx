...
import { SchlaglochMeldenViewDialog } from '@/components/dialogs/SchlaglochMeldenViewDialog';
...
const SCHLAGLOCHMELDEN_FIELDS = [
  { key: 'foto', label: 'Foto des Schlaglochs', type: 'file' },
  { key: 'standort', label: 'Standort des Schlaglochs', type: 'geo' },
  { key: 'schweregrad', label: 'Schweregrad', type: 'lookup/radio', options: [{ key: 'gering', label: 'Gering (kleine Unebenheit)' }, { key: 'mittel', label: 'Mittel (deutliches Schlagloch)' }, { key: 'hoch', label: 'Hoch (gefährliches Schlagloch)' }] },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'meldedatum', label: 'Meldedatum und -uhrzeit', type: 'date/datetimeminute' },
  { key: 'melder_vorname', label: 'Vorname (Melder/in)', type: 'string/text' },
  { key: 'melder_nachname', label: 'Nachname (Melder/in)', type: 'string/text' },
  { key: 'melder_email', label: 'E-Mail-Adresse (Melder/in)', type: 'string/email' },
  { key: 'melder_telefon', label: 'Telefonnummer (Melder/in)', type: 'string/tel' },
];

const ENTITY_TABS = [
  { key: 'schlagloch_melden', label: 'Schlagloch melden', pascal: 'SchlaglochMelden' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('schlagloch_melden');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'schlagloch_melden': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'schlagloch_melden': return (data as any).schlaglochMelden as SchlaglochMelden[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'schlagloch_melden': return SCHLAGLOCHMELDEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'schlagloch_melden': return {
        create: (fields: any) => LivingAppsService.createSchlaglochMeldenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateSchlaglochMeldenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteSchlaglochMeldenEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'schlagloch_melden' || dialogState?.entity === 'schlagloch_melden') && (
        <SchlaglochMeldenDialog
          open={createEntity === 'schlagloch_melden' || dialogState?.entity === 'schlagloch_melden'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'schlagloch_melden' ? handleUpdate : (fields: any) => handleCreate('schlagloch_melden', fields)}
          defaultValues={dialogState?.entity === 'schlagloch_melden' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['SchlaglochMelden']}
          enablePhotoLocation={AI_PHOTO_LOCATION['SchlaglochMelden']}
        />
      )}
      {viewState?.entity === 'schlagloch_melden' && (
        <SchlaglochMeldenViewDialog
          open={viewState?.entity === 'schlagloch_melden'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'schlagloch_melden', record: r }); }}
        />
      )}
